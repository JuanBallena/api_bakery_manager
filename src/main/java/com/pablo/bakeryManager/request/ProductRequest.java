...
@Setter
@Getter
public class ProductRequest implements GetRequest<Product> {
	...
	public Boolean parameterIdCategoryIsPositive() {
		return validator.positiveNumber(this.idCategory);
	}
	...
	public Boolean hasFilterParameters() {
		return this.parameterIdCategoryIsPositive() || this.parameterNameHasText() || this.parameterVisibleNotNull();
	}
	...
	@Override
	public FilterSpecification<Product> getFilterSpecification() {
		
		FilterSpecification<Product> filterSpecification = new FilterSpecification<Product>();
		List<Filter> filters = new ArrayList<Filter>();
		
		if (this.parameterIdCategoryIsPositive()) filters.add(new Filter(Product.CATEGORY, this.idCategory));
		if (this.parameterNameHasText()) filters.add(new Filter(Product.NAME, this.name));
		if (this.parameterVisibleNotNull()) filters.add(new Filter(Product.VISIBLE, this.visible));		
